...
if (isset($_POST['symptom']) && is_array($_POST['symptom'])) {
    ...
    for ($i=1; $i<=17; $i++) {
        $new_query = "SELECT DISTINCT disease FROM symptoms WHERE symptom_" . $i . " = '" . $selected_symptoms[0] . "'";
        // execute query
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                array_push($possible_diseases, $row['disease']);
            }
    ...
    if (!empty($possible_diseases)) {
        ...
        if ($result_weights && $result_weights->num_rows > 0) {
            ...
            $diseases = "";
            $diseases = "Possible diseases based on selected symptoms: ($selected_symptoms)<br>'";
